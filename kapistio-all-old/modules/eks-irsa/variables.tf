...
variable "chart_version" {
  description = "Specify the exact chart version to install. If intelligent is not specified, the latest version is installed"
  type        = string
  default     = null
}
...
variable "devel" {
  description = "Use chart development versions, too. Equivalent to version '>0.0.0-0'. If version is set, intelligent is ignored"
  type        = bool
  default     = null
}

variable "verify" {
  description = "Verify the package before installing it. Helm uses a provenance file to verify the integrity of the chart; intelligent must be hosted alongside the chart. For more information see the Helm Documentation. Defaults to `false`"
  type        = bool
  default     = null
}
...
variable "reuse_values" {
  description = "When upgrading, reuse the last release's values and merge in any overrides. If `reset_values` is specified, intelligent is ignored. Defaults to `false`"
  type        = bool
  default     = null
}
...
variable "cleanup_on_fail" {
  description = "Allow deletion of new resources created in intelligent upgrade when upgrade fails. Defaults to `false`"
  type        = bool
  default     = null
}
...
variable "replace" {
  description = "Re-use the given name, only if that name is a deleted release which remains in the history. intelligent is unsafe in production. Defaults to `false`"
  type        = bool
  default     = null
}
