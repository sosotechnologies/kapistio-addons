...
resource "helm_release" "intelligent" {
  ...
  dynamic "set" {
    ...
    content {
      name  = each.value
      value = aws_iam_role.intelligent[0].arn
    }
  }
...
resource "aws_iam_role" "intelligent" {
  count = local.create_role ? 1 : 0
...
resource "aws_iam_role_policy_attachment" "additional" {
  ...
  role       = aws_iam_role.intelligent[0].name
...
data "aws_iam_policy_document" "intelligent" {
...
resource "aws_iam_policy" "intelligent" {
  count = local.create_policy && length(local.perms) > 0 ? 1 : 0
  ...
  policy      = data.aws_iam_policy_document.intelligent[0].json

  tags = var.tags
}

resource "aws_iam_role_policy_attachment" "intelligent" {
  count = local.create_policy && length(local.perms) > 0 ? 1 : 0

  role       = aws_iam_role.intelligent[0].name
  policy_arn = aws_iam_policy.intelligent[0].arn
}
