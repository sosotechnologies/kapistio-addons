...
resource "helm_release" "cafanwi" {
  ...
  dynamic "set" {
    ...
    content {
      name  = each.value
      value = aws_iam_role.cafanwi[0].arn
    }
  }
...
resource "aws_iam_role_policy_attachment" "additional" {
  ...
  role       = aws_iam_role.cafanwi[0].name
...
data "aws_iam_policy_document" "cafanwi" {
...
resource "aws_iam_policy" "cafanwi" {
  count = local.create_policy && length(local.perms) > 0 ? 1 : 0
  ...
  policy      = data.aws_iam_policy_document.cafanwi[0].json

  tags = var.tags
}

resource "aws_iam_role_policy_attachment" "cafanwi" {
  count = local.create_policy && length(local.perms) > 0 ? 1 : 0

  role       = aws_iam_role.cafanwi[0].name
  policy_arn = aws_iam_policy.cafanwi[0].arn
}
